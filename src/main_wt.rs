...
impl News {
    ...
    fn walk(&self, client: &mut client::Client, cookie: &Cookie) -> String {
        ...
        match document.find(Class("article-text")).iter().nth(0) {
            Some(story) => {
                if let Some(x) = story.find(Class("bigtext")).first() {
                    x.find(Name("p")).iter().fold(String::new(), | text, node | text + &node.text())
                } else {
                    String::new()
                }
            },
...
fn main() {
    ...
    let address = args.nth(1).unwrap_or("http://www.washingtontimes.com/news/politics/".to_owned());
    ...
    let items = document.find(Class("story")).iter().map(|node|
        node.find(Class("article-headline")).first().map(|x| x.find(Name("a")).first().map(|news| {
            News::new(news.text(), 
                      "http://www.washingtontimes.com/".to_owned()+news.attr("href").unwrap_or(""), 
                      node.find(Name("p")).first().and_then(|x|Some(x.text())).unwrap_or("".to_owned()))
         }))).filter(|x| x.is_some()).map(|x|x.unwrap()).filter(|x|x.is_some()).map(|x|x.unwrap()).collect::<Vec<_>>();

    let setcookie = match res.headers.get::<SetCookie>() {
        Some(t) => t.clone(),
        _ => SetCookie(Vec::new()),
    };
